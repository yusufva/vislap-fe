        ...
        <v-col class="d-flex flex-column align-center justify-center">
            <v-card class="w-50 px-6" variant="outlined" color="blue-darken-4">
                ...
                <v-text-field v-model="password" label="Password" variant="outlined" prepend-inner-icon="mdi-shield-key"
                            ...
                            @click:append-inner="showpass = !showpass" density="compact" @keyup.enter="onRegist(), loading=true"></v-text-field>
                <v-text-field v-model="con_password" label="Confirm Password" variant="outlined" prepend-inner-icon="mdi-shield-key"
                            :append-inner-icon="showcon ? 'mdi-eye' : 'mdi-eye-off'"
                            :type="showcon ? 'text' : 'password'"
                            @click:append-inner="showcon = !showcon" density="compact" @keyup.enter="onRegist(), loading=true"></v-text-field>
                <v-card-action>
                    <v-btn class="mb-4 text-body-2" variant="flat" color="blue-darken-4" density="comfortable" block @click="onRegist(), loading=true" :loading="loading">Register</v-btn>
                    <div class="text-red text-caption">{{message}}</div>
                </v-card-action>
            </v-card>
            <div class="text-body-2 mt-2">
                    Sudah punya akun? <router-link class="text-decoration-none text-blue-darken-3" to="/login">Login</router-link>
                </div>
        </v-col>
    </v-row>
</template>
