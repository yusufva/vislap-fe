<template lang="">
    <div class="h-screen mx-auto text-center" v-if="loading" >
        <v-progress-circular class="mt-16" :size="128" :width="12" color="blue-darken-3" indeterminate></v-progress-circular>
    </div>
        ...
        <v-container class="w-75">
            ...
            <v-row class="mt-8">
                ...
                <v-card height="320">
                    ...
                    <v-card-text class="mx-4 mt-n8 text-body-2">{{items.nama}}</v-card-text>
                    <div class="text-right mr-4 mb-2">
                            <v-menu :close-on-content-click="false">
                                <template v-slot:activator="{props}">
                                    <v-btn color="blue-darken-3" icon="mdi-cart-arrow-down" location="right bottom" position="absolute" variant="text" v-bind="props" @click="count=1, itemId=items.id, itemPrice=items.harga">
                                    </v-btn>
                                </template>
                                <v-card>
                                    <div class="mt-2 d-flex flex-row align-center">
                                        <v-btn icon="mdi-minus-circle" variant="plain" color="blue-darken-3" @click="minusCount()"></v-btn>
                                        <v-card-text class="text-center">{{count}}</v-card-text>
                                        <v-btn icon="mdi-plus-circle" variant="plain" color="blue-darken-3" @click="plusCount()"></v-btn>
                                    </div>
                                    <v-card-actions class="mt-n4">
                                        <v-btn class="caption mx-auto" color="blue-darken-3" size="small" icon="mdi-cart-plus" variant="plain" @click="addToCart(), cartLoading=true" :loading="cartLoading"></v-btn>
                                    </v-card-actions>
                                </v-card>
                            </v-menu>
                    </div>
    ...
    methods: {
        ...
        async addToCart() {
            if (useAuthStore().isLoggedIn === false) {
                ...
                try {
                    const submitCart = await axios.post(useEnvStore().apiUrl + 'cart', {
